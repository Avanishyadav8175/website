...
export default function TableFormFields({
  ...
  const validTopicOptions = topicOptions.filter(
    ({ value }) =>
      topics.find(({ _id }) => _id === value)?.category === category
  );

  const validSubTopicOptions = subTopicOptions.filter(
    ({ value }) =>
      subTopics.find(({ _id }) => _id === value)?.category === category && subTopics.find(({ _id }) => _id === value)?.topic === topic
  );

  const validSubSubTopicOptions = subSubTopicOptions.filter(
    ({ value }) =>
      subSubTopics.find(({ _id }) => _id === value)?.category === category
      && subSubTopics.find(({ _id }) => _id === value)?.topic === topic
      && subSubTopics.find(({ _id }) => _id === value)?.subTopic === subTopic
  );
